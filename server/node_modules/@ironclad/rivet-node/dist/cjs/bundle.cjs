    ...
    socket.on("message", async (data) => {
      try {
        ...
        await (0, import_ts_pattern.match)(message).with({ type: "run" }, async () => {
          var _a;
          const { graphId, inputs, runToNodeIds } = message.data;
          await ((_a = options.dynamicGraphRun) == null ? void 0 : _a.call(options, { client: socket, graphId, inputs, runToNodeIds }));
...
var NodeDatasetProvider = class _NodeDatasetProvider extends import_rivet_core4.InMemoryDatasetProvider {
  static async fromDatasetsFile(datasetsFilePath) {
    try {
  ...
  static async fromProjectFile(projectFilePath) {
    const dataFilePath = projectFilePath.replace(/\.rivet-project$/, ".rivet-data");
    return _NodeDatasetProvider.fromDatasetsFile(dataFilePath);
  }
};
